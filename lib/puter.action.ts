...
export const createProject = async ({ item, visibility = "private" }: CreateProjectParams): Promise<DesignItem | null | undefined> => {
    ...
    if (!resolvedSource) {
        console.warn("Failed to resolve source image URL for project:", projectId);
        return null
    }
    ...
    try {
        ...
        if (!response.ok) {
            console.error("Failed to save project: Server responded with", await response.text());
            return null;
        }
...
export const getProjects = async () => {
    ...
    try {
        ...
        if (!response.ok) {
            console.error("Failed to fetch projects: Server responded with", await response.text());
            return [];
        }
